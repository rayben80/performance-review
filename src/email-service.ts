// Cloudflare Workers용 이메일 서비스 모듈 (Fetch API 기반)
...
export class EmailService {
  ...
  constructor(config: EmailConfig) {
    this.config = config;
    console.log('✅ Email service initialized for Cloudflare Workers');
  }

  // 회원가입 신청 알림 (관리자에게)
  async notifySignupRequest(userData: { name: string; email: string; role: string }) {
    const template: EmailTemplate = {
      to: 'admin@company.com', // 관리자 이메일
  ...
  private async sendEmail(template: EmailTemplate): Promise<boolean> {
    try {
      // Cloudflare Workers 환경에서는 외부 이메일 API 사용
      // 개발 환경에서는 콘솔에 이메일 내용 출력 (시뮬레이션)
      console.log('📧 이메일 발송 시뮬레이션:');
      console.log(`받는 사람: ${template.to}`);
      console.log(`제목: ${template.subject}`);
      console.log(`발송자: ${this.config.gmailUser}`);
      console.log(`시간: ${new Date().toLocaleString('ko-KR')}`);
      console.log('---');
      
      // 실제 프로덕션에서는 SendGrid, Mailgun 등의 API 사용
      // 예시: await this.sendViaExternalAPI(template);
  ...
  // 실제 이메일 발송을 위한 외부 API 사용 예시 (추후 구현)
  private async sendViaExternalAPI(template: EmailTemplate): Promise<boolean> {
    // SendGrid, Mailgun, Resend 등의 API 사용 예시
    /*
    const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: this.config.gmailUser, name: this.config.systemName },
